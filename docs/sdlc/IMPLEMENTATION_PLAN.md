...
| ID | Task | Acceptance criteria | Notes |
|---|---|---|---|
| SS3.1 | Generate a `PreToolUse` hook config (`.claude/settings.local.json` or `.claude/hooks/hooks.json`, whichever this repo's Claude Code version actually supports — confirm during implementation) that denies `Edit`/`Write` outside the active task packet's `write_paths`, with explicit Windows-path normalization (this machine is Windows; don't let a backslash/forward-slash mismatch silently no-op the check). | A manual test: spawn the SS1.1 implementer agent with a task packet whose `write_paths` excludes some file, confirm the agent's attempt to write it is denied before execution, not just discouraged in the prompt. | Depends on SS1.1, SS0.3. This is the task most likely to reveal a real Claude Code capability gap (hooks documentation vs. what's actually configurable in this repo's version) — if the mechanism doesn't work as expected, report the gap rather than faking the acceptance criterion. |
