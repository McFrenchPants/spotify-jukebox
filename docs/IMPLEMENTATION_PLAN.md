...
- **P4.3 — Now playing & queue view (SSE-driven)**: subscribes to `/api/events` (P1.5) for now-playing/queue updates — no client-side polling; animated crossfade transition on track change (art/title/artist), elapsed/remaining progress indicator; manual-refresh fallback affordance if the SSE connection drops and doesn't auto-recover within a few seconds.
  - Backend gap found while starting this task: SSE only pushes *deltas* on change, so a freshly-loaded client has no way to get the *current* now-playing/queue state before the next change fires. Adds two small public (unauthenticated) read endpoints as a prerequisite: `GET /api/now-playing` (last-seen state from P1.5's poller) and `GET /api/queue` (the `queue_entries` local mirror from P3.4, same data `GET /api/admin/queue` already exposes, just without admin auth — read-only queue contents aren't sensitive).
...
- **P4.6 — Admin panel UI**: PIN entry screen, settings form (P3.2), queue moderation actions (P3.4), device selector (P1.4), QR code display/print view for the guest URL. Lives behind the **Settings** tab (see P4.8).
  - Accept: manual walkthrough of PIN login → change a setting → see it take effect on the guest side.
- **P4.8 — Navigation restructure to 4-tab IA** (added after user review of the working P4.1–P4.5 build; see DESIGN_SPEC §9b): introduces `react-router-dom` routes + a persistent bottom tab bar (**Now Playing** / **Find Music** / **Playback History** / **Settings**), splitting the single stacked page built across P4.1–P4.5 into separate routed screens. Now Playing becomes icon-only transport controls (previous/pause-resume/skip, no text buttons) + volume + Up Next, with search and leaderboard/recent moved off it onto their own tabs. App display name changes to "French's Jukebox" (`AppShell` header, `index.html` title, PWA manifest). Backend gains `POST /api/playback/previous` (mirrors the existing skip endpoint, same `skip` trust-mode capability) and a small public `GET /api/artist/:id` (genres/image/follower count via Spotify's `GET /v1/artists/{id}`) for the new "About the artist" panel on Now Playing — the latter requires the backend's track-shaping to start retaining Spotify artist IDs (currently discarded in favor of a joined display-name string).
  - Lyrics considered and explicitly declined by the user (no official Spotify API support) — not built.
  - Accept: all previously-working P4.1–P4.5 functionality still works, now reachable via its respective tab; bottom nav persists across tab switches; Now Playing tab shows no search/leaderboard content.
